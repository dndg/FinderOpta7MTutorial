...
between the Opta and a Finder 7M energy meter.In particular, we are going to
learn how to correctly convert values read from the 7M registers to valid
floating point values and upload them to the cloud.
...
to power the Opta and make sure to correctly setup the RS-485 serial
connection between the Opta and the 7M. You can refer to the following diagram
while connecting your Opta device to the Finder 7M energy meter via the RS-485
interface.
...
7M energy meter. Additionally, it shows how it is possible to use the
